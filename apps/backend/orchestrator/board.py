...
async def run_board_session(
    ...
    evidence_packet = await gather_board_evidence(
        session, question, workspace_id, workspace_context=board_ctx.text[:2000]
    )
    research_trace = evidence_packet.to_trace()
    board_ctx.trace["research"] = research_trace
    board_ctx.trace["research_sources_count"] = research_trace.get("passed_to_researcher", 0)
    ...
    context_trace = {**board_ctx.trace, "researcher": researcher_debug}
    board.synthesis = {
        **synthesis,
